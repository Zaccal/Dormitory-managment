import { Theme, useTheme } from '@/hooks/ui/theme-provider'
import { cn } from '@/lib/utils'
import {
    getThemeIcon,
    getThemeIconStyles,
    getThemeInfo,
} from '@/lib/themeOptionHelpers'
...
const ThemeOption = ({ theme, onClick }: IThemeOption) => {
    ...
    return (
        <div
            ...
            className={cn(
                'w-full px-4 py-2 cursor-pointer rounded-md flex items-center gap-5',
                isActive
            )}
        >
            <div
                className={cn(
                    'rounded-md px-12 py-8',
                    getThemeIconStyles(theme)
                )}
            >
                {getThemeIcon(theme)}
            </div>
